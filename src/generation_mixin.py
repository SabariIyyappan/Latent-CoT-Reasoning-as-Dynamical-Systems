...
class LatentGenerationMixin(GenerationMixin):
    ...
    # GPT-2 based models don't support the Cache class API introduced in
    # newer transformers versions — set to False so generation_mixin falls
    # back to the legacy tuple-based past_key_values path.
    ...
    def generate(
        ...
        **kwargs,
        ...
        # Version-agnostic cache preparation:
        #   transformers < 4.57  → signature requires 'device' as 6th arg
        #   transformers >= 4.57 → signature requires 'generation_mode' as 3rd arg, no 'device'
        import inspect as _insp
        _cache_params = list(_insp.signature(self._prepare_cache_for_generation).parameters.keys())
        if 'device' in _cache_params:
        ...
        # (removed in transformers >= 5.6 — guard with getattr)
        if (
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        # Version-agnostic cache_position init:
        #   transformers <  5.6  → GenerationMixin exposes `_get_initial_cache_position`
        #   transformers >= 5.6  → method was removed; inline the equivalent logic
        if hasattr(self, "_get_initial_cache_position"):
            model_kwargs = self._get_initial_cache_position(cur_len, input_ids.device, model_kwargs)
        else:
            if "inputs_embeds" in model_kwargs and not getattr(self.config, "is_encoder_decoder", False):
                _cp = torch.ones_like(
                    model_kwargs["inputs_embeds"][0, :, 0], dtype=torch.int64
                ).cumsum(0) - 1
            elif "decoder_inputs_embeds" in model_kwargs and getattr(self.config, "is_encoder_decoder", False):
                _cp = torch.ones_like(
                    model_kwargs["decoder_inputs_embeds"][0, :, 0], dtype=torch.int64
                ).cumsum(0) - 1
            else:
                _cp = torch.arange(cur_len, device=input_ids.device, dtype=torch.int64)
            _pkv = model_kwargs.get("past_key_values")
            if _pkv is not None:
                if hasattr(_pkv, "get_seq_length"):
                    _past_len = _pkv.get_seq_length() or 0
                elif isinstance(_pkv, tuple):
                    _past_len = _pkv[0][0].shape[2]
                else:
                    _past_len = getattr(_pkv, "seen_tokens", 0) or 0
                if _past_len:
                    _cp = _cp[_past_len:]
            model_kwargs["cache_position"] = _cp
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # Advance cache_position so prepare_inputs_for_generation slices
            # only the NEW token on the next step (transformers >= 5.6 removed
            # the auto-increment that lived in _update_model_kwargs_for_generation).
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        has_kvs = isinstance(past_key_values, tuple) or (
            isinstance(past_key_values, DynamicCache) and (
                past_key_values.get_seq_length() if hasattr(past_key_values, 'get_seq_length')
                else past_key_values.seen_tokens
            ) > 0
        )
        if has_kvs:
            if do_repeat:
